...
На этом уроке вы  создадите ваших первых акторов и познакомитесь с основами [Akka.NET](http://getakka.net/).

## Ключевые идеи / общая информация

Во время этого урока вы получите практический опыт работы с акторами в консольном приложении.  Мы создадим систему акторов и несколько акторов в рамках этой системы. 

Напишем двух акторов, один з которых будет читать из консоли, а другой обрабатывать входящие данные и писать результат консоль.

### Что такое актор?

"Актор" - аналог человека, выполняющего какую-то роль в системе. Он является некоей сущностью или объектом, проводить какие-то действия и общаться с другими людьми.

> Мы предполагаем, что вам знакомы идеи объектно ориентированного программирования (ООП).  Модель акторов очень похожа на объектно ориентированную модель - в рамках ООП все является объектами, в рамках модели акторов ***все являются акторами***.

Повторяйте про себя: все является актороми. Все являются актороми. Все являются актороми! Представьте любую систему как иерархию людей, задачи  в которой разбиваются на части и делегируются до тех пор, пока не станут достаточно маленькими для качественного выполнения даже одним актором.

В рамках ООП вы стараетесь дать каждому объекту простую и четко определенную цель существования. В принципе, модель акторов очень похожа, только теперь за достижение цели отвечает актор.
...
Более детально мы разберем сообщения на следующем уроке. Все что вам пока сейчас знать - сообщения можно отправить другому актору при помощи метода `Tell()`
...
### Что могут делать акторы?

Все что вы запрограммируете. Серьезно :)

Вы создаете акторов, заставляете их обрабатывать получаемые сообщения, а для обработки сообщений акторы могут выполнить любой код. Делать запросы к базе данных, писать в файл, изменять внутренние переменные, все что нам можен вдруг понадобиться.
...
1. Менять свое поведение и обрабатывать следующие сообщения по-другому алгоритму
...
Все типы акторов наследуются от `UntypedActor`, но пока не важно. Детальный разбор разных типов акторов будет проведен чуть позже.

В первом блоке все ваши акторы будут наследниками [`UntypedActor`](http://getakka.net/docs/Working%20with%20actors#untypedactor-api "Akka.NET - UntypedActor API").

### Как создать актора?

Для создания актора необходимо знать две основные вещи:

1. Все акторы создаются в рамках определенного контекста. 
...
Подробнее `Props` разберем третьем уроке, так что пока не особо парьтесь по этому поводу. В коде примеров вы увидите несколько `Props`, этого дложно быть достаточно для понимания, как создать актора с их помощью.
